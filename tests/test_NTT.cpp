#include <gtest/gtest.h>
#include "ModularInt.hpp"
#include "SmallestUInt.hpp"
#include "NTT.hpp"
#include "NTTUtils.hpp"


TEST(NTT, NTTContextTest) {

    // We fix a small modulus for testing purposes
    constexpr uint64_t Q = 17;
    constexpr size_t N = 4;
    using Poly = ModularInt<Q, N>;

    constexpr uint64_t root = 13;  // Primitive 4-th root of unity mod 17
    
    NTTContext<Q, N> ctx(root);
    EXPECT_EQ(ctx.root, root);
    EXPECT_EQ(ctx.inv_root, 4);  // Inverse of root mod Q (since 13^3 = 4 mod 17)
}

TEST(NTT, RoundTrip1) {
    // We fix a small modulus for testing purposes
    constexpr uint64_t Q = 17;
    constexpr size_t N = 4;
    using Poly = ModularInt<Q, N>;

    constexpr uint64_t root = 13;  // Primitive 4-th root of unity mod 17
    
    NTTContext<Q, N> ctx(root);
    
    Poly p;
    p[0] = 1;
    p[1] = 2;
    p[2] = 3;
    p[3] = 4;
    forward_ntt(p, ctx); // This should become [10, 6, 15, 7]

    ASSERT_EQ(p[0], 10);
    ASSERT_EQ(p[1], 6);
    ASSERT_EQ(p[2], 15);
    ASSERT_EQ(p[3], 7);

    inverse_ntt(p, ctx); // This should return to [1, 2, 3, 4]
    
    ASSERT_EQ(p[0], 1);
    ASSERT_EQ(p[1], 2);
    ASSERT_EQ(p[2], 3);
    ASSERT_EQ(p[3], 4);
}




// TEST(NTT, Roundtrip) {

//     /// ML-KEM-768 Parameters
//     constexpr uint64_t Q = 3329;
//     constexpr size_t N = 256;
//     using Poly = ModularInt<Q, N>;

//     Poly p;
//     for (size_t i = 0; i < N; ++i)
//         p[i] = i % Q;

//     Poly p_orig = p;  // Save original

//     NTTContext<Q, N> ctx(17);  // root is primitive N-th root

//     std::cout << "Prior to NTT, INTT" << std::endl;
//     for (size_t i = 0; i < N; ++i) {
//         std::cout << "p[" << i << "] = " << p[i] << ", expected = " << p_orig[i] << "\n";
//     }

//     forward_ntt(p, ctx);
//     inverse_ntt(p, ctx);

//     std::cout << "After NTT, INTT" << std::endl;


//     for (size_t i = 0; i < N; ++i) {
//         std::cout << "p[" << i << "] = " << p[i] << ", expected = " << p_orig[i] << "\n";
//     }

//     for (size_t i = 0; i < N; ++i) 
//         ASSERT_EQ(p[i], p_orig[i]) << "Mismatch at index " << i;
    
    
// }